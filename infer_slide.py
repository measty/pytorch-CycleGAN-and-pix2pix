...
def construct_slide(slide_path, mask, patch_size=256, proc_fn=None, resolution=0, units='level', stride=None, save_path=None, **kwargs):
    ...
    for i, tile in tqdm(enumerate(patch_extractor), total=len(patch_extractor)):
        ...
        if y+rec.shape[0] > canvas_shape[0] or x+rec.shape[1] > canvas_shape[1]:
            print("patch out of bounds, shifting.")
...
if __name__ == '__main__':
    ...
    if var_thresh == 0:
        var_thresh = None
    back_heuristic = opt.bckgrnd_heuristic
    ...
    for slide in slides[:min(len(slides), opt.num_test)]:
        ...
        construct_slide(slide, mask, proc_fn=proc_fn, resolution=resolution, units=units, stride=stride, save_path=save_path, back_heuristic=back_heuristic, var_thresh=var_thresh)
